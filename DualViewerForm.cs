...
namespace SpecimenFX17.Imaging
{
    public class DualViewerForm : Form
    {
        ...
        private void BuildUI()
        {
            ...
            _pic1.MouseDown += (s, e) => { if (e.Button == MouseButtons.Left) { _pt1 = MapToImage(_pic1, e.Location); _pic1.Invalidate(); _plot.Invalidate(); } };
            _pic2.MouseDown += (s, e) => { if (e.Button == MouseButtons.Left) { _pt2 = MapToImage(_pic2, e.Location); _pic2.Invalidate(); _plot.Invalidate(); } };
        ...
        private void Plot_Paint(object? sender, PaintEventArgs e)
        {
            ...
            try
            {
                ...
                if (_cube1 != null && _pt1.HasValue) s1 = _cube1.GetSpectrum(_pt1.Value.Y, _pt1.Value.X);
                if (_cube2 != null && _pt2.HasValue) s2 = _cube2.GetSpectrum(_pt2.Value.Y, _pt2.Value.X);

                processSpec(s1!); processSpec(s2!);

                if (yMin == float.MaxValue || float.IsNaN(yMin) || float.IsInfinity(yMin)) { yMin = 0; yMax = 1; }
                if (Math.Abs(yMin - yMax) < 1e-6f) { yMin -= 0.5f; yMax += 0.5f; }
            ...
            catch (Exception ex)
            {
                ...
                g.DrawString($"Error renderizando gráfica: {ex.Message}", font, Brushes.Salmon, 10, 10);
            }
        }
