...
{
    /// <summary>
    /// Calculadora científica espectral con visualización de resultados en 5 pestañas:
    ///   1. Imagen BLI pseudocolor
    ///   2. Histograma de valores
    ///   3. Perfil horizontal (fila central)
    ///   4. Perfil vertical  (columna central)
    ///   5. Estadísticas numéricas
    ///
    /// Sintaxis:
    ///   B{λ}   → banda más cercana a λ nm   ej: B{680}
    ///   B[n]   → banda por índice 1-based   ej: B[1]
    ///   Ops:   + - * / ^ (potencia)
    ///   Funcs: sqrt log log2 log10 exp abs sin cos tan asin acos atan
    ///          round floor ceil min(a,b) max(a,b) pow(a,b)  PI  E
    /// </summary>
    public class SpectralCalculatorForm : Form
    {
        ...
        private void BuildUI()
        {
            ...
            // Toolbar paleta+progreso (Dock Top)
            var toolPanel = BuildToolPanel();

            // TabControl de resultados (Fill)
            _tabs = BuildTabs();
        ...
        private void BuildLeftPanel(Panel p)
        {
            ...
            _lstBands.DoubleClick += (_, _) =>
            {
                ...
                double wl = _cube.Header.Wavelengths[_lstBands.SelectedIndex];
                _txtFormula.SelectedText = $"B{{{wl:F1}}}";
                _txtFormula.Focus();
            };
        ...
        private TabControl BuildTabs()
        {
            ...
            tc.TabPages.Add(pgImg);

            // Pestaña 1: Histograma
            ...
            pgHisto.Controls.Add(_tabHisto);
            tc.TabPages.Add(pgHisto);

            // Pestaña 2: Perfil horizontal
            ...
            var phBar = new Panel { Dock = DockStyle.Top, Height = 34, BackColor = Color.FromArgb(18, 18, 28) };
            ...
            phContainer.Controls.Add(_tabProfileH);
            phContainer.Controls.Add(phBar);
            pgPH.Controls.Add(phContainer);
            tc.TabPages.Add(pgPH);

            // Pestaña 3: Perfil vertical
            ...
            var pvBar = new Panel { Dock = DockStyle.Top, Height = 34, BackColor = Color.FromArgb(18, 18, 28) };
            ...
            pvContainer.Controls.Add(_tabProfileV);
            pvContainer.Controls.Add(pvBar);
            pgPV.Controls.Add(pvContainer);
            tc.TabPages.Add(pgPV);
            ...
            tc.TabPages.Add(pgStats);

            return tc;
        }

        // ═════════════════════════════════════════════════════════════════════
        //  PREVIEW (píxel central)
        // ═════════════════════════════════════════════════════════════════════
        ...
        private async void BtnCalc_Click(object? s, EventArgs e)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    for (int line = 0; line < _cube.Lines; line++)
                    {
                        ...
                        if (line % 8 == 0)
                        {
                            int pct = (line + 1) * 100 / _cube.Lines;
                            Invoke(() =>
                            {
                                _progress.Value = pct;
                                _lblStatus.Text = $"Calculando… {pct} %";
                            });
                        }
                    }
                }
            ...
            _resultBitmap = RenderFloatArray(_resultData, (BliColormap)_cmbColormap.SelectedIndex, formula, stats);
        ...
        private void BtnSave_Click(object? s, EventArgs e)
        {
            ...
            {
                Filter = "PNG (*.png)|*.png|TIFF (*.tif)|*.tif",
                FileName = "resultado_espectral"
            };
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                var fmt = dlg.FileName.EndsWith(".tif", StringComparison.OrdinalIgnoreCase)
                          ? ImageFormat.Tiff : ImageFormat.Png;
        ...
        private void DrawHistogram()
        {
            ...
            for (int l = 0; l < _cube.Lines; l++)
                ...
                {
                    float v = _resultData[l, c];
                    if (float.IsNaN(v) || float.IsInfinity(v)) continue;
                    if (v < histoXMin || v > histoXMax) continue;
                    int bin = Math.Clamp((int)((v - histoXMin) / histoRange * (bins - 1)), 0, bins - 1);
                    counts[bin]++;
                    total++;
                }

            int maxCount = counts.Max();
            int histoYMax2 = zoom != null ? (int)(zoom.Y1 * maxCount) : maxCount;
            int histoYMin2 = zoom != null ? (int)(zoom.Y0 * maxCount) : 0;
            ...
            // Barras del histograma coloreadas por valor
            float binW = (float)plot.Width / bins;
            float yRangeH = histoYMax2 - histoYMin2; if (yRangeH < 1) yRangeH = 1;
            ...
            // Línea de media (solo si está en el rango visible)
            if (stats.Mean >= histoXMin && stats.Mean <= histoXMax)
            {
                float meanX = plot.Left + (stats.Mean - histoXMin) / histoRange * plot.Width;
                using var mp = new Pen(Color.FromArgb(220, 255, 255, 80), 1.5f) { DashStyle = DashStyle.Dash };
                g.DrawLine(mp, meanX, plot.Top, meanX, plot.Bottom);
                using var sf2 = new Font("Consolas", 7.5f);
                using var sb2 = new SolidBrush(Color.FromArgb(180, 255, 255, 80));
                g.DrawString($"μ={stats.Mean:G4}", sf2, sb2, meanX + 3, plot.Top + 2);
            }
        ...
        // ── Perfil horizontal (fila) ──────────────────────────────────────────
        private void DrawProfileH()
        {
            if (_resultData == null) return;
        ...
        // ── Perfil vertical (columna) ─────────────────────────────────────────
        private void DrawProfileV()
        {
            if (_resultData == null) return;
        ...
        // ── Gráfico lineal genérico ───────────────────────────────────────────
        private Bitmap DrawLinePlot(int w, int h, float[] values,
            string xLabel, string yLabel, string title, Color color,
            ZoomRange? zoom = null)
        {
            var bmp = new Bitmap(w, h);
            using var g = Graphics.FromImage(bmp);
            ...
            const int padL = 58, padR = 20, padT = 28, padB = 40;
            var plot = new Rectangle(padL, padT, w - padL - padR, h - padT - padB);
            if (plot.Width < 20 || plot.Height < 20) return bmp;

            // Rango base completo
            float fullVMin = values.Where(v => !float.IsNaN(v) && !float.IsInfinity(v)).DefaultIfEmpty(0).Min();
            float fullVMax = values.Where(v => !float.IsNaN(v) && !float.IsInfinity(v)).DefaultIfEmpty(1).Max();
            float margin = (fullVMax - fullVMin) * 0.05f; if (margin < 1e-10f) margin = 0.05f;
            fullVMin -= margin; fullVMax += margin;
            ...
            float xRng = xMax - xMin; if (xRng < 1e-10f) xRng = 1f;
            ...
            // Construir puntos solo en el rango visible
            ...
            {
                // Área bajo la curva
                var fillPts = new PointF[pts.Count + 2];
                fillPts[0] = new PointF(pts[0].X, plot.Bottom);
                pts.CopyTo(fillPts, 1);
                fillPts[^1] = new PointF(pts[^1].X, plot.Bottom);
                using (var fb = new SolidBrush(Color.FromArgb(25, color.R, color.G, color.B)))
                    g.FillPolygon(fb, fillPts);

                // Sombra + línea
                using (var sh = new Pen(Color.FromArgb(50, color.R, color.G, color.B), 3f))
                    g.DrawLines(sh, pts.ToArray());
                using (var lp = new Pen(color, 1.6f) { LineJoin = LineJoin.Round })
            ...
            using var tf = new Font("Segoe UI", 8.5f, FontStyle.Bold);
            using var tb = new SolidBrush(Color.FromArgb(170, 180, 215));
            g.DrawString(title, tf, tb, padL, 6);

            // Indicador de zoom activo
            if (zoom != null)
            {
                using var zf = new Font("Segoe UI", 7.5f, FontStyle.Bold);
                using var zb = new SolidBrush(Color.FromArgb(200, 255, 200, 80));
                g.DrawString("🔍 Zoom activo — clic derecho para resetear", zf, zb, padL + 4, padT + 4);
            }
        ...
        private void FillStats(ResultStats s, string formula)
        {
            ...
            {
                _tabStats.SelectionColor = Color.FromArgb(140, 160, 200);
                _tabStats.AppendText($"  {label,-20}");
                _tabStats.SelectionColor = Color.FromArgb(230, 240, 210);
            ...
            Row("P95%:", s.P95.ToString("G8"));
            Row("P98%:", s.P98.ToString("G8"));
            Row("P99%:", s.P99.ToString("G8"));
        }
        ...
        private record ResultStats(float Min, float Max, float Mean, float Median,
            float Std, float Skew, float P1, float P2, float P5, float P25,
            float P50, float P75, float P95, float P98, float P99, int ValidCount);
        ...
        private ResultStats CalcStats(float[,] data)
        {
            ...
            return new ResultStats(
                Min: vals[0], Max: vals[^1], Mean: mean, Median: median,
                Std: std, Skew: skew,
                P1: Pct(1), P2: Pct(2), P5: Pct(5), P25: Pct(25),
                P50: Pct(50), P75: Pct(75), P95: Pct(95), P98: Pct(98), P99: Pct(99),
                                        ...
                                        string formula, ResultStats stats)
        {
            ...
            var pixels = new byte[stride * lines];

            for (int l = 0; l < lines; l++)
            {
                int row = l * stride;
                for (int c2 = 0; c2 < samples; c2++)
                {
                    float v = data[l, c2];
                    float t = float.IsNaN(v) || float.IsInfinity(v)
                              ? 0f : Math.Clamp((v - lo) / range, 0f, 1f);
                    var (r2, g2, b2) = GetColor(t, colormap);
                    int o = row + c2 * 3;
                    pixels[o] = b2; pixels[o + 1] = g2; pixels[o + 2] = r2;
                }
            }
            ...
            // Colorbar + etiqueta fórmula
            using var gfx = Graphics.FromImage(bmp);
            gfx.SmoothingMode = SmoothingMode.AntiAlias;
            int barH = Math.Min(120, lines - 30), barW = 14;
            int bx = samples - barW - 8, by = 15;
            for (int i = 0; i < barH; i++)
            {
                float t = 1f - (float)i / barH;
                var (r2, gc2, b2) = GetColor(t, colormap);
            ...
            string lbl = formula.Length > 60 ? formula[..57] + "…" : formula;
            using var ff = new Font("Consolas", 8f, FontStyle.Bold);
            using var fg = new SolidBrush(Color.FromArgb(160, 0, 0, 0));
            var fsz = gfx.MeasureString(lbl, ff);
            gfx.FillRectangle(fg, 5, lines - fsz.Height - 5, fsz.Width + 4, fsz.Height + 2);
            gfx.DrawString(lbl, ff, wb, 7, lines - fsz.Height - 4);
        ...
        private static string ToNum(float v)
        {
        ...
        private static string EvalPower(string expr) =>
            Regex.Replace(expr, @"([\d.eE+\-]+)\^([\d.eE+\-]+)", m =>
            {
        ...
        private static double EvalSimple(string e) =>
            Convert.ToDouble(new DataTable().Compute(e.Trim(), null));

        private static string Fmt(double v) =>
            v.ToString("R", System.Globalization.CultureInfo.InvariantCulture);

        private static int FindFunc(string expr, string name)
        {
            int idx = 0;
            while (true)
            {
                int pos = expr.IndexOf(name, idx, StringComparison.OrdinalIgnoreCase);
                if (pos < 0) return -1;
                int after = pos + name.Length;
                if (after < expr.Length && expr[after] == '(' &&
                    (pos == 0 || !char.IsLetterOrDigit(expr[pos - 1]))) return pos;
                idx = pos + 1;
            }
        }

        private static int FindClose(string expr, int openPos)
        {
            int d = 0;
            for (int i = openPos; i < expr.Length; i++)
            {
                if (expr[i] == '(') d++;
                else if (expr[i] == ')') { d--; if (d == 0) return i; }
            }
            return -1;
        }

        private static int TopComma(string s)
        {
            int d = 0;
        ...
        private int FindClosestBand(double wl)
        {
            if (_cube.Header.Wavelengths.Count == 0) return 0;
            return _cube.Header.Wavelengths
                .Select((w, i) => (diff: Math.Abs(w - wl), i))
        ...
        private static void DrawPlotGrid(Graphics g, Rectangle plot)
        {
            ...
            using var bp = new Pen(Color.FromArgb(60, 255, 255, 255));
            g.DrawRectangle(bp, plot);
        }

        private static void DrawPlotAxesXY(Graphics g, Rectangle plot,
            float xMin, float xMax, float yMin, float yMax,
            string xLabel, string yLabel)
        {
            using var tf = new Font("Consolas", 7.5f);
            using var tb = new SolidBrush(Color.FromArgb(155, 155, 195));
            using var af = new Font("Segoe UI", 8f, FontStyle.Italic);
            using var ab = new SolidBrush(Color.FromArgb(115, 125, 165));
            ...
            for (int i = 0; i <= 6; i++)
            {
                float v = xMin + xRng * i / 6;
                float x = plot.Left + (float)i / 6 * plot.Width;
                string lb = v >= 10000 || (Math.Abs(v) < 0.001f && v != 0)
                            ? v.ToString("0.0e0") : v.ToString("G4");
                var sz = g.MeasureString(lb, tf);
                g.DrawString(lb, tf, tb, x - sz.Width / 2, plot.Bottom + 2);
            }
            var xts = g.MeasureString(xLabel, af);
            g.DrawString(xLabel, af, ab, plot.Left + plot.Width / 2f - xts.Width / 2f, plot.Bottom + 20);

            // Eje Y
            for (int i = 0; i <= 5; i++)
            {
                float v = yMin + yRng * i / 5;
                float y = plot.Bottom - (float)i / 5 * plot.Height;
                string lb = v >= 10000 || (Math.Abs(v) < 0.001f && v != 0)
                            ? v.ToString("0.0e0") : v.ToString("G4");
                var sz = g.MeasureString(lb, tf);
                g.DrawString(lb, tf, tb, plot.Left - sz.Width - 3, y - sz.Height / 2);
            }
            var st = g.Save();
            g.TranslateTransform(10, plot.Top + plot.Height / 2f);
            g.RotateTransform(-90);
            g.DrawString(yLabel, af, ab, -40, -6);
            g.Restore(st);
        }
        ...
        // ═════════════════════════════════════════════════════════════════════
        //  ZOOM INTERACTIVO  (arrastrar = zoom,  clic derecho = resetear)
        // ═════════════════════════════════════════════════════════════════════

        /// <summary>
        /// Asocia zoom por arrastre a un PictureBox.
        /// - redraw   : acción que regenera el bitmap con el zoom actual
        /// - reset    : acción que borra el zoom y redibuja
        /// - setZoom  : guarda el nuevo ZoomRange
        /// - getZoom  : lee el ZoomRange actual (para Paint overlay)
        ///
        /// El zoom se calcula convirtiendo las coordenadas de pantalla a
        /// valores de datos usando el área de trazado fija (padL=58, padR=20, padT=28, padB=40).
        /// </summary>
        private void AttachZoom(PictureBox pb,
            Action redraw, Action reset,
            Action<ZoomRange?> setZoom, Func<ZoomRange?> getZoom)
        {
            const int padL = 58, padR = 20, padT = 28, padB = 40;
            ...
            pb.MouseUp += (s, e) =>
            {
                ...
                int dx = Math.Abs(e.X - dragStart.X);
                int dy = Math.Abs(e.Y - dragStart.Y);
                ...
                float nx0 = Math.Clamp((float)(Math.Min(dragStart.X, e.X) - plotLeft) / plotW, 0f, 1f);
                float nx1 = Math.Clamp((float)(Math.Max(dragStart.X, e.X) - plotLeft) / plotW, 0f, 1f);
                float ny1 = Math.Clamp(1f - (float)(Math.Min(dragStart.Y, e.Y) - plotTop) / plotH, 0f, 1f);
                float ny0 = Math.Clamp(1f - (float)(Math.Max(dragStart.Y, e.Y) - plotTop) / plotH, 0f, 1f);
                if (nx1 - nx0 < 0.01f || ny1 - ny0 < 0.01f) return;

                // Obtener el rango actual (puede ser un zoom previo)
                var cur = getZoom();

                // Necesitamos conocer el rango de datos actual del plot.
                // Lo leemos del bitmap actual: buscamos los valores de los ejes.
                // Como no los almacenamos por separado, guardamos como fracción normalizada
                // y dejamos que DrawLinePlot/DrawHistogram lo interprete.
                // Aquí guardamos las fracciones; cada método las convierte a datos reales.
                setZoom(new ZoomRange(nx0, nx1, ny0, ny1));
                redraw();
            };
            ...
            {
                if (!dragging) return;
                int x1 = Math.Min(dragStart.X, dragCur.X);
                int y1 = Math.Min(dragStart.Y, dragCur.Y);
                int rw = Math.Abs(dragCur.X - dragStart.X);
                int rh = Math.Abs(dragCur.Y - dragStart.Y);
                if (rw < 3 || rh < 3) return;

                using var fill = new SolidBrush(Color.FromArgb(40, 80, 200, 255));
                e.Graphics.FillRectangle(fill, x1, y1, rw, rh);
                using var pen = new Pen(Color.FromArgb(200, 80, 200, 255), 1.5f) { DashStyle = DashStyle.Dash };
                e.Graphics.DrawRectangle(pen, x1, y1, rw, rh);
        ...
        // ── Helpers de UI ─────────────────────────────────────────────────────
        private static void AddLbl(Control p, string text, int cy,
            bool bold = false, Color? color = null, float size = 8.5f)
        {
