...
namespace SpecimenFX17.Imaging
{
    ...
    public sealed class PixelShape : SelectionShape
    {
        ...
        public override string ShortLabel => $"({Pt.X},{Pt.Y})";
    ...
    public sealed class RectShape : SelectionShape
    {
        ...
        public override string ShortLabel => $"({Rect.X},{Rect.Y}) {Rect.Width}×{Rect.Height}";
        public override string LegendIcon => "▭";
        public override bool[,] GetMask(int L, int S)
        {
            var m = new bool[L, S];
    ...
    public sealed class PolygonShape : SelectionShape
    {
        ...
        {
            get {
                int n=Vertices.Length, cx=Vertices.Sum(p=>p.X)/n, cy=Vertices.Sum(p=>p.Y)/n;
                return $"Pol. {n}v c({cx},{cy})";
            }
        }
        public override string LegendIcon => "⬟";
        public override bool[,] GetMask(int L, int S)
    ...
    public sealed class CircleShape : SelectionShape
    {
        ...
        public override string ShortLabel => $"Círculo c({Center.X},{Center.Y}) r={Radius}";
        public override string LegendIcon => "○";
        public override bool[,] GetMask(int L, int S)
        ...
        public override void DrawOn(Graphics g)
        {
            ...
            using var rp = new Pen(Color.FromArgb(110,Color.R,Color.G,Color.B),1f)
                           { DashStyle=DashStyle.Dot };
            g.DrawLine(rp, Center.X, Center.Y, Center.X+Radius, Center.Y);
    ...
    public sealed class FreehandShape : SelectionShape
    {
        ...
        {
            get {
                int w=Points.Max(p=>p.X)-Points.Min(p=>p.X);
                int h=Points.Max(p=>p.Y)-Points.Min(p=>p.Y);
                return $"Lasso {w}×{h} px";
            }
        }
        public override string LegendIcon => "✏";
